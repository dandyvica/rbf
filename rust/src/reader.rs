...
//!
//!    // load our layout
//!    let layout = Layout::<UTF8Mode>::new("./tests/test.xml").unwrap();
//!
//!    // create reader
...
use crate::record::{ReadMode, Record};
...
pub struct ReaderStats {
    ...
    pub nb_chars_read: u64,
    /// number of lines read so far
    pub nb_lines_read: u64,
}
...
pub struct Reader<T> {
    ...
    /// input file size
    pub file_size: u64,
    /// useful stats kept when reading
...
impl<T> Reader<T> {
    ...
    pub fn new(rbf_file: &str, layout: Layout<T>) -> Reader<T> {
    ...
    pub fn next(&mut self) -> Option<(ReaderStats, &mut Record<T>)>
    where
        Record<T>: ReadMode,
    {
        // record ID from line
        ...
        // new stats to return
        //let mut stats = ReaderStats{ chars_read: 0, nb_chars_read: 0, nb_lines_read: 0 };
        ...
        loop {
            ...
            match self.bufreader.read_line(&mut self.line) {
                ...
                Ok(chars_read) => {
                    if chars_read == 0 {
                        ...
                        self.stats.chars_read = chars_read;
                        self.stats.nb_chars_read += chars_read as u64;
                        self.stats.nb_lines_read += 1;
                    }
                }
                // error reading bytes
                Err(why) => panic!(
            ...
            // check if we'd to skip this line
            if self.ignore_line.as_str() != "" && self.ignore_line.is_match(&self.line) {
                continue;
            }

            // get the record ID using mapper
            rec_id = (self.layout.mapper.func)(&self.line);
            //rec_id = self.map_call(&self.line);

            // record ID could not exist
            match self.layout.contains_record(&rec_id) {
                true => break,
                false => {
                    if self.lazyness == ReaderLazyness::Strict {
                        panic!(
                            "couldn't find record ID {} in file {}",
                            rec_id, self.rbf_file
                        );
                    } else {
                        continue;
                    }
        ...
        return Some((self.stats.clone(), rec));
    }
